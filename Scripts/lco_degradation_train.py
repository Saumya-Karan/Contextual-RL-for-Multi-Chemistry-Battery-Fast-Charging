"""
LCO Degradation Model  (v2 — tiny model for small dataset)
============================================================
Dataset: Sandia LCO_1-6, _Reg files, 5/15/25/35/45 degC
...
Key changes from v1:
  ...
  4. Filter out near-flat 45C data for LCO_1/2/3 (SOH > 0.98 = noise)
     LCO barely degrades at 45C in Sandia test window
...
OUTPUT_DIR  = r"lco_degradation_outputs"
os.makedirs(OUTPUT_DIR, exist_ok=True)

TRAIN_CELLS = ['LCO_1','LCO_2','LCO_3','LCO_4']
VAL_CELLS   = ['LCO_5']
TEST_CELLS  = ['LCO_6']
...
def main():
    ...
    print('='*60); print('STEP 1: Loading Sandia LCO'); print('='*60)
    df = load_sandia_chemistry(SANDIA_ROOT, 'LCO')
    if len(df)==0:
    ...
    torch.save(model.state_dict(),
               os.path.join(OUTPUT_DIR,'lco_degradation_model.pt'))
    with open(os.path.join(OUTPUT_DIR,'lco_degradation_stats.pkl'),'wb') as f:
        pickle.dump(stats,f)
    with open(os.path.join(OUTPUT_DIR,'lco_degradation_history.pkl'),'wb') as f:
        pickle.dump({'history':history,
                     'metrics':{'train':m_tr,'val':m_v,'test':m_te}},f)

    print(f'\nSaved: {OUTPUT_DIR}/')
    print('\n'+'='*60); print('SUMMARY'); print('='*60)
    print(f'  Chemistry   : LCO (Sandia LCO_1-6)')
...
def load_lco_degradation(output_dir):
    model = DegradationMLP()
    model.load_state_dict(torch.load(
        os.path.join(output_dir,'lco_degradation_model.pt'),map_location='cpu'))
    model.eval()
    with open(os.path.join(output_dir,'lco_degradation_stats.pkl'),'rb') as f:
        stats = pickle.load(f)
    return model, stats
