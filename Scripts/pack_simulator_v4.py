...
class PackSimulator:
    ...
    def _init_state(self, SOC_init, SOH_init):
        ...
        # Coulomb-counting SOC tracker (chemistry-agnostic, no OCV dependency)
        self.soc_cc      = float(SOC_init)   # running integral of charge delivered
        self.prev_soc_cc = float(SOC_init)   # for velocity computation
    ...
    def step(self, I_cell, mdot):
        ...
        # Update Coulomb-counting SOC (exact integral, no OCV dependence)
        # Convention: positive I_cell = charging for all chemistries
        # LFP dataset uses negative current for charging but we track absolute charge
        dSOC_cc = I_cell * DT / (self.params['C_nom'] * 3600.0)
        self.prev_soc_cc = self.soc_cc
        self.soc_cc = float(np.clip(self.soc_cc + dSOC_cc, 0.0, 1.0))
    ...
    def _state(self):
        """
        11-dimensional state vector, size-independent.

        Elements:
          0  SOC_max        — max cell SOC (surrogate-predicted)
          ...
          9  soc_cc         — Coulomb-counted SOC (OCV-independent)
                              Critical for LFP whose flat OCV plateau makes
                              voltage-based SOC inference unreliable.
         10  dsoc_cc_dt     — SOC charging velocity (Coulomb-counted)
                              Tells the policy how fast charge is accumulating.
                              Prevents premature current tapering.
        """
        theta_norm = float(self.params['theta']) / 2.0
        n_norm     = float(self.n_cells) / float(N_CELLS_MAX)
        # SOC velocity: change in Coulomb-counted SOC this step (× 100 for scale)
        dsoc_dt    = (self.soc_cc - self.prev_soc_cc) * 100.0
        return np.array([
